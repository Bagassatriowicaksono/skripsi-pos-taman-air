...
<div id="home" class="home">
    <div class="container mt-5 py-5">
        ...
        <div class="row">
            <div class="col-md-9">
                <button type="button" class="btn btn-green btn-md mt-2" data-toggle="modal"
                ...
                <a href="<?= $urlexcel;?>&cetak=print" target="_blank" class="btn btn-danger mt-2 btn-md ml-1">
                    <i class="fa fa-print"></i> Cetak
                </a>
                <?php }else{ if (!empty($this->input->get('kasir', true))) {?>
                <a href="<?= $urlexcel;?>&cetak=print" target="_blank" class="btn btn-primary mt-2 btn-md ml-1">
                    <i class="fa fa-print"></i> Cetak
                </a>
                <?php } else {?> -->
                <a href="<?= $urlexcel;?>?cetak=print" target="_blank" class="btn btn-secondary mt-2 btn-md ml-1">
                    <i class="fa fa-print"></i> Cetak
                </a>
                <!-- <?php }}?> -->
                <!-- <?php if($this->input->get('kasir')){?>
                <a href="<?= base_url('laporan/produk?kasir='.$this->input->get('kasir',true));?>" class="btn btn-warning mt-2 btn-md ml-1">
                    <i class="fa fa-refresh"></i> Refresh
                </a>
                <?php }else{?> -->
                <a href="<?= base_url('laporan/produk');?>" class="btn btn-warning mt-2 btn-md ml-1">
                    <i class="fa fa-refresh"></i> Refresh
                </a>
                <?php }?>
            </div>
        ...
        <div class="card card-rounded">
            <div class="card-header bg-success text-white">
                <?= $title_web;?>
                <?= $periode;?>
                <a href="<?= base_url('laporan/laris'); ?>" class="btn btn-green ml-2"> Menu Terlaris</a>
                    ...
                    <table id="example1" class="table table-bordered table-sm table-striped table" width="100%">
                        ...
                        <!-- <tfoot>
                            <tr>
                                ...
                                <th><?= $total->qty;?></th>
                                <th colspan="7">Rp<?= number_format($total->h ?? 0);?>,-</th>
                            </tr>
                        </tfoot> -->
                    </table>
                </div>
                    ...
                    <div class="form-group">
                        ...
                        <select class="form-control" name="kasir">
                            ...
                            <?php 
                                ...
                                foreach($kasir as $r){
                            ?>
                            <option value="<?= $r->id;?>" <?php if($this->session->userdata('ses_level') != 'Admin') { echo 'selected'; }?>><?= $r->nama_user;?></option>
                            <?php }?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label for="">Tanggal Start</label>
                        <input type="datetime-local" required class="form-control" value="<?= $this->input->get('a')?>" name="a"
                            placeholder="">
                    </div>
                    <div class="form-group">
                        <label for="">Tanggal End</label>
                        <input type="datetime-local" required class="form-control" value="<?= $this->input->get('b')?>" name="b"
                            placeholder="">
                    </div>
                </div>
...
<?php
    if(!empty($this->input->get('a'))){
        ...
        if (!empty($this->input->get('kasir', true))) {
            $url .= '&kasir='.$this->input->get('kasir', true);
        }
    }else{
    ...
    tabel = $('#example1').DataTable({
        ...
        "columns": [{
            ...
            
            {
                data: 'harga',
                "render": function(data, type, row, meta) {
                    return 'Rp' + number_format(row.harga * row.qty) + ',-';
                }
            },
            {
                'data': 'atas_nama'
